...
import type { LookupResult } from '../../../main/server';
...
const status = ref<LookupResult | { latency: null; status: null } | null>(null);
    ...
    <template v-slot:top-right v-if="status">
      <div class="status">
        <div
          ...
          v-if="status.status"
          :title="
            status.status?.players?.sample?.map((s: any) => s.name).join('\n')
          "
        >
        ...
        <div
          class="latency"
          :title="status?.latency ? `${status?.latency} ms` : 'No connection'"
        >
          <Icon
            :path="
              (() => {
